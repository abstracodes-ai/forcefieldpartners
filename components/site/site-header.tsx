  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      <div className="container py-4">
        <div className="field-line dark-panel flex items-center justify-between rounded-[2rem] border border-white/10 px-4 py-3.5 shadow-forcefield sm:px-6 sm:py-4">
          <Link href="/" className="flex items-center" onClick={() => setOpen(false)}>
            <BrandLockup mode="dark" size="md" className="sm:pr-4" />
          </Link>

          <nav className="hidden items-center gap-5 text-sm text-slate-300 xl:gap-6 lg:flex">
            {navigation.map((item) => (
              ...
              <nav className="flex flex-col gap-3 text-sm text-slate-200">
                {navigation.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="rounded-2xl px-3 py-2 font-medium hover:bg-white/6 hover:text-white"
